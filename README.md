...
| 用途 | 说明 |
|------|------|
| 前端对照 | 在同一类工具链（Vite + TypeScript）下对比 **React** 与 **Vue** 的写法与能力差异。 |
...
- **跨栈对照（前端）**：React / Vue 示例均为纯前端单页、无强制共用后端；对照时可关注状态与副作用、内置能力（如 Portal / Teleport、错误边界 / `onErrorCaptured`）等子项目文档中的章节。
...
- 新增其他前端目录（如 Svelte、Solid）时，继续沿用「一目录一工程」模式即可。
