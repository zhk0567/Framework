...
| **后端** | [`Back-end/`](Back-end) | [`Back-end/Node/`](Back-end/Node) 为 **Fastify、NestJS** 及 **Express、Koa、Hapi、Restify、AdonisJS（占位）、Strapi（占位）、Directus（占位）、Hono、Elysia（Bun）**；[`Back-end/Go/`](Back-end/Go) 为多种 **Go** Web 框架；各子工程在 **HTTP/JSON**（如 `/api/*`）形态上对齐，便于对照中间件、路由注册与响应结构。 |

**规模一览**：**34** 个前端 + **8** 个全栈 + **20** 个后端（[`Back-end/Node`](Back-end/Node) **11**、[`Back-end/Go`](Back-end/Go) **9**），共 **62** 个可独立运行的子工程（**RedwoodJS** 因官方 CLI 对 Node 版本与 yarn 等约束，未纳入生成树，见 [FRAMEWORK-GAP-LIST.md](FRAMEWORK-GAP-LIST.md) 第一节）。
