        ...
        for (let t = 0; t < tokens.length; t++) {
            ...
            if (token.kind === TOKEN_LIT) {
                ...
                while (pos < input.length && !symbolSet.has(input[pos]))
                    pos++;
                if (pos === start)
                    return null; // empty PART invalid
...
class SyntaxInterpreter {
    constructor() {
        /** @internal */ this._patterns = [];
        /** @internal */ this._cache = new Map();
    }
    add(defs) {
    ...
    interpret(name) {
        ...
        if (bestPattern !== null) {
            ...
            this._cache.set(name, { pattern: null, parts: kernel.emptyArray });
        }
        return interpretation;
    }
}
...
class AttributeParser {
    constructor() {
        /** @internal */ this._interpreter = kernel.resolve(ISyntaxInterpreter);
        /** @internal */ this._container = kernel.resolve(kernel.IContainer);
        /** @internal */ this._patternHandlers = {};
        /** @internal */ this._pendingDefs = [];
    ...
    registerPattern(patterns, Type) {
        ...
        for (const def of patterns) {
            if (handlers[def.pattern] != null)
                throw createMappedError(89 /* ErrorNames.attribute_pattern_duplicate */, def.pattern);
    ...
    parse(name, value) {
        ...
        return handlerInfo.instance[pattern](name, value, interpretation.parts);
    }
}
function attributePattern(...patternDefs) {
    return function decorator(target, context) {
        context.metadata[kernel.registrableMetadataKey] = AttributePattern.create(patternDefs, target);
        return target;
    };
}
...
DotSeparatedAttributePattern[_a] = {
    [kernel.registrableMetadataKey]: /*@__PURE__*/ AttributePattern.create([{ pattern: 'PART.PART', symbols: '.' }, { pattern: 'PART.PART.PART', symbols: '.' }], DotSeparatedAttributePattern)
};
...
RefAttributePattern[_b] = {
    [kernel.registrableMetadataKey]: /*@__PURE__*/ AttributePattern.create([{ pattern: 'ref', symbols: '' }, { pattern: 'PART.ref', symbols: '.' }], RefAttributePattern)
};
...
EventAttributePattern[_c] = {
    [kernel.registrableMetadataKey]: /*@__PURE__*/ AttributePattern.create([{ pattern: 'PART.trigger:PART', symbols: '.:' }, { pattern: 'PART.capture:PART', symbols: '.:' }], EventAttributePattern)
};
...
AtPrefixedTriggerAttributePattern[_e] = {
    [kernel.registrableMetadataKey]: /*@__PURE__*/ AttributePattern.create([{ pattern: '@PART', symbols: '@' }, { pattern: '@PART:PART', symbols: '@:' }], AtPrefixedTriggerAttributePattern)
};
...
class OneTimeBindingCommand {
    ...
    build(info, exprParser, attrMapper) {
        ...
        return { type: itPropertyBinding, from: exprParser.parse(value, etIsProperty), to: target, mode: 1 /* InternalBindingMode.oneTime */ };
    }
}
...
class ToViewBindingCommand {
    ...
    build(info, exprParser, attrMapper) {
        ...
        return { type: itPropertyBinding, from: exprParser.parse(value, etIsProperty), to: target, mode: 2 /* InternalBindingMode.toView */ };
    }
}
...
class FromViewBindingCommand {
    ...
    build(info, exprParser, attrMapper) {
        ...
        return { type: itPropertyBinding, from: exprParser.parse(value, etIsProperty), to: target, mode: 4 /* InternalBindingMode.fromView */ };
    }
}
...
class TwoWayBindingCommand {
    ...
    build(info, exprParser, attrMapper) {
        ...
        return { type: itPropertyBinding, from: exprParser.parse(value, etIsProperty), to: target, mode: 6 /* InternalBindingMode.twoWay */ };
    }
}
...
class SpreadValueBindingCommand {
    get ignoreAttr() { return false; }
    build(info) {
        return { type: itSpreadValueBinding, target: info.attr.target, from: info.attr.rawValue };
    }
}
...
class TemplateCompiler {
        ...
        };
        {
            if (compiledDef.instructions.some(row => row.some(instr => {
                if (instr.type === itHydrateElement) {
                    return instr.res === compiledDef.name
                        || instr.res?.name === compiledDef.name;
                }
                return false;
            }))) {
                // eslint-disable-next-line no-console
    ...
    compileSpread(requestor, attrSyntaxs, container, target, targetDef) {
        ...
        for (; ii > i; ++i) {
                    ...
                    if (bindingCommand == null) {
                        ...
                        instructions.push({
                            ...
                            instruction: expr == null
                                ? { type: itSetProperty, value: attrValue, to: bindable.name }
                                : { type: itInterpolation, from: expr, to: bindable.name }
                        });
                    }
                    else {
                        instructions.push({
                            type: itSpreadElementProp,
                            instruction: bindingCommand.build({ node: target, attr: attrSyntax, bindable, def: elDef }, context._exprParser, context._attrMapper)
                        });
                    }
                    continue;
                }
            ...
            if (attrDef !== null) {
                ...
                attrBindableInstructions = this._compileCustomAttributeBindables(target, attrDef, attrSyntax, attrValue, bindingCommand, context, 
            ...
            if (bindingCommand == null) {
                ...
                if (expr != null) {
                    instructions.push({
                        ...
                        to: context._attrMapper.map(target, attrTarget) ?? kernel.camelCase(attrTarget)
                    });
                }
                else {
                    switch (attrTarget) {
    ...
    _compileSurrogate(el, context) {
        ...
        for (let i = 0, ii = attrs.length; i < ii; ++i) {
            ...
            if (invalidSurrogateAttribute[attrSyntax.target]) {
                throw createMappedError(702 /* ErrorNames.compiler_invalid_surrogate_attr */, attrs[i].name);
            }
        }
        ...
        const { tcInstructions, attrInstructions, plainAttrInstructions } = this._classifyAttributes(el, null, kernel.emptyArray, context, /* generateStaticAttrInstructions */ true);
        ...
        if (tcInstructions != null && tcInstructions.length > 0) {
            ...
            throw createMappedError(703 /* ErrorNames.compiler_no_tc_on_surrogate */, tcName);
        }
        // 4. Combine: custom attributes first, then plain attribute instructions
        if (attrInstructions != null) {
            return attrInstructions.concat(plainAttrInstructions ?? kernel.emptyArray);
        }
        return plainAttrInstructions ?? kernel.emptyArray;
    }
    // overall flow:
    ...
    _compileLet(el, context) {
        ...
        for (; ii > i; ++i) {
            ...
            if (bindingCommand !== null) {
                if (attrSyntax.command === 'bind') {
                    letInstructions.push({
                        ...
                        to: kernel.camelCase(realAttrTarget)
                    });
                }
                else {
            ...
            letInstructions.push({
                type: itLetBinding,
                from: expr === null ? expressionParser.createPrimitiveLiteralExpression(realAttrValue) : expr,
                to: kernel.camelCase(realAttrTarget)
            });
        }
        context.rows.push([{
    ...
    _compileElement(el, context) {
        ...
        const captures = elDef?.capture ? [] : kernel.emptyArray;
        // Validate slot usage
        if (elName === 'slot') {
        ...
        if (isCustomElement) {
            ...
            elementInstruction = {
                ...
                props: (elBindableInstructions ?? kernel.emptyArray),
                projections: null,
    ...
    _classifyAttributes(el, elDef, captures, context, generateStaticAttrInstructions = false) {
        ...
        for (; ii > i; ++i) {
                ...
                if (bindable !== void 0) {
                    if (bindingCommand === null) {
                        ...
                        (elBindableInstructions ??= []).push(expr == null
                            ? { type: itSetProperty, value: realAttrValue, to: bindable.name }
                            : { type: itInterpolation, from: expr, to: bindable.name });
                    }
                    else {
                        (elBindableInstructions ??= []).push(bindingCommand.build({ node: el, attr: attrSyntax, bindable, def: elDef }, context._exprParser, context._attrMapper));
                    }
                    removeAttr();
                    {
            ...
            attrDef = context._findAttr(realAttrTarget);
            if (attrDef !== null) {
                attrBindableInstructions = this._compileCustomAttributeBindables(el, attrDef, attrSyntax, realAttrValue, bindingCommand, context, 
            ...
            if (bindingCommand === null) {
                ...
                if (expr != null) {
                    ...
                    (plainAttrInstructions ??= []).push({
                        type: itInterpolation,
                        from: expr,
                        to: context._attrMapper.map(el, realAttrTarget) ?? kernel.camelCase(realAttrTarget)
                    });
                }
        ...
        for (let i = 0; i < valueLength; ++i) {
            ...
            if (ch === 92 /* Char.Backslash */) {
                ...
                if (command === null) {
                    ...
                    instructions.push(command.build({ node, attr: attrSyntax, bindable, def: attrDef }, context._exprParser, context._attrMapper));
                }
    ...
    _shouldReorderAttrs(el, instructions) {
        ...
        return nodeName === 'INPUT' && orderSensitiveInputType[el.type] === 1
            || nodeName === 'SELECT' && (el.hasAttribute('multiple')
                || instructions?.some(i => i.type === itPropertyBinding && i.to === 'multiple'));
    }
    /** @internal */
...
exports.itAttributeBinding = itAttributeBinding;
exports.itHydrateAttribute = itHydrateAttribute;
exports.itHydrateElement = itHydrateElement;
exports.itHydrateLetElement = itHydrateLetElement;
exports.itHydrateTemplateController = itHydrateTemplateController;
exports.itInterpolation = itInterpolation;
exports.itIteratorBinding = itIteratorBinding;
exports.itLetBinding = itLetBinding;
exports.itListenerBinding = itListenerBinding;
exports.itMultiAttr = itMultiAttr;
exports.itPropertyBinding = itPropertyBinding;
exports.itRefBinding = itRefBinding;
exports.itSetAttribute = itSetAttribute;
exports.itSetClassAttribute = itSetClassAttribute;
exports.itSetProperty = itSetProperty;
exports.itSetStyleAttribute = itSetStyleAttribute;
exports.itSpreadElementProp = itSpreadElementProp;
exports.itSpreadTransferedBinding = itSpreadTransferedBinding;
exports.itSpreadValueBinding = itSpreadValueBinding;
exports.itStylePropertyBinding = itStylePropertyBinding;
exports.itTextBinding = itTextBinding;
exports.templateCompilerHooks = templateCompilerHooks;
