...
const builtInSymbols = new Set(Object.getOwnPropertyNames(Symbol).map(k => Symbol[k]).filter(s => kernel.isSymbol(s) && s !== Symbol.iterator));
function doNotCollect(object, key) {
    if (key === 'constructor'
        ...
        || key === '$observers'
        || kernel.isSymbol(key) && builtInSymbols.has(key)
...
function observeTrackableMethodDependencies(connectable, instance, options) {
    ...
    const dependencies = kernel.isFunction(deps) ? [deps] : deps;
...
const objectHandler = {
    get(target, key, receiver) {
        ...
        return function trackableMethod(...args) {
            ...
            const rawThisArg = kernel.isObject(thisArg)
                ? getRaw(thisArg)
                : thisArg;
            observeTrackableMethodDependencies(current, rawThisArg, options);
            const useProxy = options.deps == null;
            const invokedThis = useProxy && kernel.isObject(rawThisArg)
                ? wrap(rawThisArg)
                : rawThisArg;
...
function astTrack(targetOrOptionsOrDependency, ...rest) {
    if (kernel.isFunction(targetOrOptionsOrDependency) && isClassMethodDecoratorContext$1(rest[0])) {
    ...
    if (typeof targetOrOptionsOrDependency === 'string') {
    ...
    else if (kernel.isFunction(targetOrOptionsOrDependency)) {
        ...
        if (Object.prototype.hasOwnProperty.call(configuredOptions, 'deps')) {
            options.deps = configuredOptions.deps == null
                ? void 0
                : kernel.isFunction(configuredOptions.deps)
...
function computed(targetOrOptionsOrDependency, ...rest) {
    ...
    if (kernel.isFunction(targetOrOptionsOrDependency) && isClassMethodDecoratorContext(rest[0])) {
    ...
    return function universalDecorator(target, context) {
        ...
        const getterOptions = kernel.isObject(targetOrOptionsOrDependency)
            ? targetOrOptionsOrDependency
            : {};
        if (!kernel.isObject(targetOrOptionsOrDependency)) {
...
function observeDeep(obj, requestor) {
    function walk(obj, connectable) {
        ...
        if (!kernel.isObject(raw)) {
            return;
        }
        if (kernel.isArray(raw)) {
...
class ObserverLocator {
    constructor() {
        ...
        /** @internal */ this._expressionParser = kernel.resolve(kernel.optional(expressionParser.IExpressionParser));
    }
    addAdapter(adapter) {
...
class Observation {
    ...
    _doWatch(obj, expressionOrGetter, callback, options) {
        ...
        const observer = kernel.isString(expressionOrGetter) ? this.oL.getExpressionObserver(obj, expressionOrGetter) : this.oL.getObserver(obj, expressionOrGetter);
        const handler = {
            handleChange
        };
        const run = () => {
...
exports.isTaskQueueEmpty = isTaskQueueEmpty;
