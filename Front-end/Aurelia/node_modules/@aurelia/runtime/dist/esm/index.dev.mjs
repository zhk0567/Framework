import { DI, isSymbol, isArray, isMap, isSet, isFunction, isObject, isObjectOrFunction, isArrayIndex, noop, areEqual, Registration, resolve, IPlatform, createLookup, isString, ILogger, optional, emptyObject } from '@aurelia/kernel';
...
/** @internal */ const getProto = Object.getPrototypeOf;
/** @internal */ const getOwnPropDesc = Object.getOwnPropertyDescriptor;
...
const errorsMap = {
    ...
    [117 /* ErrorNames.ast_track_decorator_not_a_method */]: `The @astTrack decorator can only be applied to methods, "{{0}}" is not a method.`,
    ...
    [200 /* ErrorNames.observing_expression_no_parser */]: `Trying to observe expression "{{0}}" but there is no expression parser available`,
    ...
    [227 /* ErrorNames.computed_mutating */]: `Side-effect detected in computed getter '{{0}}': mutation during evaluation caused self-dirtying. This can lead to infinite recursion. Use non-mutating operations (e.g., spread syntax) or move side effects outside the getter.`,
    [228 /* ErrorNames.computed_not_getter */]: `@computed decorator can only be used on a getter, "{{0}}" is not a getter.`,
};
 ...
 * @internal
 */
let _currentConnectable = null; // eslint-disable-line
const connectables = [];
/** @internal */
let connecting = false; // eslint-disable-line
...
function observeTrackableMethodDependencies(connectable, instance, options) {
    ...
    function astEvaluate(ast, s, e, c) {
        switch (ast.$kind) {
            ...
            case ekCallScope: {
                ...
                if (isFunction(fn)) {
                    if (c != null && fn[astTrackableMethodMarker] != null) {
                        const options = fn[astTrackableMethodMarker];
...
function isClassMethodDecoratorContext$1(value) {
    return value != null && typeof value === 'object' && 'kind' in value;
}
function astTrack(targetOrOptionsOrDependency, ...rest) {
    if (isFunction(targetOrOptionsOrDependency) && isClassMethodDecoratorContext$1(rest[0])) {
        return applyDecorator(targetOrOptionsOrDependency, rest[0], {});
    }
    const options = {};
    if (typeof targetOrOptionsOrDependency === 'string') {
        options.deps = [targetOrOptionsOrDependency, ...rest];
    }
    else if (isFunction(targetOrOptionsOrDependency)) {
        options.deps = [targetOrOptionsOrDependency];
    }
    else if (targetOrOptionsOrDependency != null) {
        const configuredOptions = targetOrOptionsOrDependency;
...
function applyDecorator(target, context, options) {
    if (context.kind !== 'method') {
        throw createMappedError(117 /* ErrorNames.ast_track_decorator_not_a_method */, String(context.name));
    }
    const trackableTarget = target;
 ...
 */
const isTaskQueueEmpty = () => queue.length === 0 && pendingAsyncCount === 0;
/**
...
function computed(targetOrOptionsOrDependency, ...rest) {
    ...
    return function universalDecorator(target, context) {
        if (context.kind === 'method') {
            ...
            if (typeof targetOrOptionsOrDependency === 'string' || typeof targetOrOptionsOrDependency === 'symbol') {
                methodOptions.deps = [targetOrOptionsOrDependency, ...rest];
            }
            else if (isFunction(targetOrOptionsOrDependency)) {
                methodOptions.deps = targetOrOptionsOrDependency;
            }
        ...
        const getterOptions = isObject(targetOrOptionsOrDependency)
            ? targetOrOptionsOrDependency
            : {};
        if (!isObject(targetOrOptionsOrDependency)) {
            getterOptions.deps = [targetOrOptionsOrDependency, ...rest];
        }
        const getterTarget = target;
        const getterContext = context;
        /* istanbul ignore next */
        if (getterContext.kind !== 'getter') {
            throw createMappedError(228 /* ErrorNames.computed_not_getter */, getterContext.name);
        }
        const cache = new WeakMap();
        return rtObjectAssign(function computedGetter() {
...
class ControlledComputedObserver {
    constructor(obj, key, getter, oL, dependencies, flush, deep) {
        this.obj = obj;
        this.key = key;
        this.getter = getter;
        this.oL = oL;
...
class ObserverLocator {
    constructor() {
        ...
        /** @internal */ this._expressionParser = resolve(optional(IExpressionParser));
    }
    addAdapter(adapter) {
    ...
    getExpressionObserver(obj, expression) {
        if (this._expressionParser == null) {
            throw createMappedError(200 /* ErrorNames.observing_expression_no_parser */, expression);
        }
        return new ExpressionObserver(obj, this, this._expressionParser.parse(expression, 'IsProperty'));
    }
    getComputedObserver(obj, key, pd, info) {
        if (info?.deps == null) {
            const observer = new ComputedObserver(obj, pd.get, pd.set, this, info?.flush);
            rtDef(obj, key, {
        ...
        return new ControlledComputedObserver(obj, key, pd.get, this, info.deps, info.flush ?? 'async', info.deep === true);
    }
...
class Observation {
    ...
    watch(obj, getterOrExpression, callback, options) {
        return this._doWatch(obj, getterOrExpression, callback, options);
    }
    /** @internal */
    _doWatch(obj, expressionOrGetter, callback, options) {
...
export { AccessorType, BindingContext, CollectionLengthObserver, CollectionSizeObserver, ComputedObserver, ConnectableSwitcher, DirtyCheckProperty, DirtyCheckSettings, DirtyChecker, ICoercionConfiguration, IDirtyChecker, INodeObserverLocator, IObservation, IObserverLocator, Observation, ObserverLocator, PrimitiveObserver, PropertyAccessor, ProxyObservable, RecurringTask, Scope, SetterObserver, Task, TaskAbortError, astAssign, astBind, astEvaluate, astTrack, astUnbind, batch, cloneIndexMap, computed, connectable, copyIndexMap, createIndexMap, getCollectionObserver, getObserverLookup, getRecurringTasks, isIndexMap, isTaskQueueEmpty, mixinNoopAstEvaluator, nowrap, observable, queueAsyncTask, queueRecurringTask, queueTask, runTasks, subscriberCollection, tasksSettled };
