...
import { createInterpolation, createAccessScopeExpression, IExpressionParser, ExpressionParser } from '@aurelia/expression-parser';
...
class BindableDefinition {
    ...
    static create(prop, def = {}) {
        ...
        return new BindableDefinition(def.attribute ?? BindableDefinition.toAttr(prop), def.callback ?? `${prop}Changed`, isString(mode) ? BindingMode[mode] ?? defaultMode : mode, def.name ?? prop, def.set ?? getInterceptor(def));
    }
}
/** @internal */
...
class AuCompose {
    ...
    _handleChangeInfo(info) {
        this._composing = onResolve(this._composing, () => onResolve(this.queue(info, void 0), (context) => {
            if (this._contextFactory._isCurrent(context)) {
                this._composing = void 0;
            }
        }));
