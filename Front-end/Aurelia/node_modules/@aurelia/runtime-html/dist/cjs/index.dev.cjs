...
const clearMetadata = (target, ...keys) => {
    for (const key of keys) {
        metadata.Metadata.delete(key, target);
    }
};
...
const errorsMap = {
    ...
    [655 /* ErrorNames.watcher_infinite_loop */]: `Possible infinitely recursive side-effect detected in a watcher`,
    ...
    [757 /* ErrorNames.rendering_mismatch_length */]: `The compiled template is not aligned with the render instructions. There are {{0}} targets and {{1}} instructions.`,
    ...
    [821 /* ErrorNames.repeat_invalid_contextual_binding_command */]: `Invalid command "{{0}}" usage with [repeat.for] option "contextual". Only "bind" or static assignment is supported.`,
    ...
    [809 /* ErrorNames.au_compose_invalid_flush_mode */]: `Invalid flushMode "{{0}}" on <au-compose />. Only "sync" or "async" allowed.`,
    ...
    [822 /* ErrorNames.hydration_target_count_mismatch */]: `SSR hydration error: manifest declares {{0}} targets but collected {{1}} from DOM.`,
    [823 /* ErrorNames.hydration_node_count_mismatch */]: `SSR hydration error: manifest declares {{0}} total nodes for views but found {{1}} nodes in DOM.`,
    [824 /* ErrorNames.hydration_view_count_mismatch */]: `SSR hydration error: manifest declares {{0}} views but items array has {{1}} elements.`,
};
...
class BindableDefinition {
    constructor(attribute, callback, mode, name, set) {
    ...
    static create(prop, def = {}) {
        ...
        return new BindableDefinition(def.attribute ?? BindableDefinition.toAttr(prop), def.callback ?? `${prop}Changed`, kernel.isString(mode) ? templateCompiler.BindingMode[mode] ?? defaultMode : mode, def.name ?? prop, def.set ?? getInterceptor(def));
    }
}
/** @internal */
...
Type, typeName, createDef, metadataKey = staticAuMetadataKey) => {
...
class ThrottleBindingBehavior {
    constructor() {
        ({ performanceNow: this._now } = kernel.resolve(kernel.IPlatform));
    }
    bind(scope, binding, delay, signals) {
        const opts = {
            type: 'throttle',
...
class CustomAttributeDefinition {
    ...
    constructor(Type, name, aliases, key, isTemplateController, bindables, noMultiBindings, watches, dependencies, containerStrategy, defaultProperty) {
        this.Type = Type;
        this.name = name;
        this.aliases = aliases;
        this.key = key;
    ...
    register(container, aliasName) {
        ...
        if (container.has(key, false)) {
            // eslint-disable-next-line no-console
            console.warn(createMappedError(154 /* ErrorNames.attribute_existed */, this.name));
        }
 ...
 */
function hydrateSSRDefinition(ssrDef) {
    const exprMap = new Map();
    for (const expr of ssrDef.expressions) {
        exprMap.set(expr.id, expr.ast);
    }
    const { instructions } = translateDefinition(ssrDef.definition, ssrDef.nestedHtmlTree, exprMap);
    return {
...
function translateDefinition(def, nestedHtmlTree, exprMap) {
    const nestedDefs = [];
    ...
    const ctx = { exprMap, nestedDefs };
    const instructions = def.instructions.map(row => row.map(ins => translateInstruction(ins, ctx)));
    return { instructions, nestedDefs };
}
function translateInstruction(ins, ctx) {
    switch (ins.type) {
        case templateCompiler.itPropertyBinding: {
            const expr = getExpr(ctx.exprMap, ins.exprId);
            const instruction = {
                type: templateCompiler.itPropertyBinding,
                from: expr,
                to: ins.to,
        ...
        case templateCompiler.itTextBinding: {
            const expressions = ins.exprIds.map(id => getExpr(ctx.exprMap, id));
            const interpolation = expressionParser.createInterpolation(ins.parts, expressions);
            const instruction = {
                type: templateCompiler.itTextBinding,
                from: interpolation,
            };
            return instruction;
        }
        case templateCompiler.itInterpolation: {
            const expressions = ins.exprIds.map(id => getExpr(ctx.exprMap, id));
            const interpolation = expressionParser.createInterpolation(ins.parts, expressions);
            const instruction = {
                type: templateCompiler.itInterpolation,
                from: interpolation,
                to: ins.to,
            };
            return instruction;
        }
        case templateCompiler.itListenerBinding: {
            const expr = getExpr(ctx.exprMap, ins.exprId);
            const instruction = {
                type: templateCompiler.itListenerBinding,
                from: expr,
                to: ins.to,
                capture: ins.capture,
        ...
        case templateCompiler.itRefBinding: {
            const expr = getExpr(ctx.exprMap, ins.exprId);
            const instruction = {
                type: templateCompiler.itRefBinding,
                from: expr,
                to: ins.to,
            };
            return instruction;
        }
        case templateCompiler.itSetProperty: {
            const instruction = {
                type: templateCompiler.itSetProperty,
                value: ins.value,
                to: ins.to,
            };
            return instruction;
        }
        case templateCompiler.itSetAttribute: {
            const instruction = {
                type: templateCompiler.itSetAttribute,
                value: ins.value ?? '',
                to: ins.to,
            };
            return instruction;
        }
        case templateCompiler.itHydrateElement: {
            const props = ins.instructions.map(i => translateInstruction(i, ctx));
            const instruction = {
                type: templateCompiler.itHydrateElement,
                res: ins.res,
                props,
                projections: null,
        ...
        case templateCompiler.itHydrateAttribute: {
            const props = ins.instructions.map(i => translateInstruction(i, ctx));
            const instruction = {
                type: templateCompiler.itHydrateAttribute,
                res: ins.res,
                alias: ins.alias,
                props,
            };
        ...
        case templateCompiler.itHydrateTemplateController: {
            ...
            const instruction = {
                type: templateCompiler.itHydrateTemplateController,
                def,
                res: ins.res,
                alias: void 0,
                props,
        ...
        case templateCompiler.itHydrateLetElement: {
            const bindings = ins.bindings.map(b => {
                const expr = getExpr(ctx.exprMap, b.exprId);
                const instruction = {
                    type: templateCompiler.itLetBinding,
                    from: expr,
                    to: b.to,
                };
                return instruction;
            ...
            const instruction = {
                type: templateCompiler.itHydrateLetElement,
                instructions: bindings,
                toBindingContext: ins.toBindingContext,
            };
            return instruction;
        }
        case templateCompiler.itIteratorBinding: {
            const forOf = getExpr(ctx.exprMap, ins.exprId);
            const props = [];
            if (ins.aux) {
                for (const aux of ins.aux) {
                    const expr = getExpr(ctx.exprMap, aux.exprId);
                    const instruction = {
                        type: templateCompiler.itMultiAttr,
                        value: expr,
                        to: aux.name,
                        command: 'bind',
                    };
            ...
            const instruction = {
                type: templateCompiler.itIteratorBinding,
                forOf,
                to: ins.to,
                props,
            };
    ...
    targetDef, rendering, compiler, platform, exprParser, observerLocator) {
        ...
        const renderSpreadInstruction = (ancestor) => {
            ...
            for (inst of instructions) {
                switch (inst.type) {
                    case templateCompiler.itSpreadTransferedBinding:
                        renderSpreadInstruction(ancestor + 1);
                        break;
                    case templateCompiler.itSpreadElementProp:
...
class SpreadValueBinding {
    ...
    _createBindings(value, unbind) {
        ...
        for (key of this.targetKeys) {
            ...
            if (key in value) {
                if (binding == null) {
                    binding = this._bindingCache[key] = new PropertyBinding(this._controller, this.l, this.oL, SpreadValueBinding._astCache[key] ??= expressionParser.createAccessScopeExpression(key, 0), this.target, key, templateCompiler.BindingMode.toView, this.strict);
                }
                binding.bind(scope);
            }
            else if (unbind) {
...
const SetPropertyRenderer = /*@__PURE__*/ renderer(class SetPropertyRenderer {
    constructor() {
        this.target = templateCompiler.itSetProperty;
    }
...
const CustomElementRenderer = /*@__PURE__*/ renderer(class CustomElementRenderer {
    constructor() {
        ...
        this.target = templateCompiler.itHydrateElement;
    }
...
const CustomAttributeRenderer = /*@__PURE__*/ renderer(class CustomAttributeRenderer {
    constructor() {
        /** @internal */ this._rendering = kernel.resolve(IRendering);
        this.target = templateCompiler.itHydrateAttribute;
    }
    render(
    /**
...
const TemplateControllerRenderer = /*@__PURE__*/ renderer(class TemplateControllerRenderer {
    constructor() {
        /** @internal */ this._rendering = kernel.resolve(IRendering);
        this.target = templateCompiler.itHydrateTemplateController;
    }
    render(renderingCtrl, target, 
    // <CustomAttributeDefinition> as we assume it's always used with the default resources resolver
    instruction, platform, exprParser, observerLocator, ssrScope) {
        /* eslint-disable prefer-const */
        let ctxContainer = renderingCtrl.container;
        let def;
        switch (typeof instruction.res) {
...
const LetElementRenderer = /*@__PURE__*/ renderer(class LetElementRenderer {
    constructor() {
        this.target = templateCompiler.itHydrateLetElement;
        LetBinding.mix();
    }
...
const RefBindingRenderer = /*@__PURE__*/ renderer(class RefBindingRenderer {
    constructor() {
        this.target = templateCompiler.itRefBinding;
        RefBinding.mix();
    }
    render(renderingCtrl, target, instruction, platform, exprParser, observerLocator) {
...
const InterpolationBindingRenderer = /*@__PURE__*/ renderer(class InterpolationBindingRenderer {
    constructor() {
        this.target = templateCompiler.itInterpolation;
...
const PropertyBindingRenderer = /*@__PURE__*/ renderer(class PropertyBindingRenderer {
    constructor() {
        this.target = templateCompiler.itPropertyBinding;
        PropertyBinding.mix();
    }
    render(renderingCtrl, target, instruction, platform, exprParser, observerLocator) {
        const container = renderingCtrl.container;
...
const IteratorBindingRenderer = /*@__PURE__*/ renderer(class IteratorBindingRenderer {
    constructor() {
        this.target = templateCompiler.itIteratorBinding;
        PropertyBinding.mix();
    }
    render(renderingCtrl, target, instruction, platform, exprParser, observerLocator) {
...
const TextBindingRenderer = /*@__PURE__*/ renderer(class TextBindingRenderer {
    constructor() {
        this.target = templateCompiler.itTextBinding;
        ContentBinding.mix();
    }
    render(renderingCtrl, target, instruction, platform, exprParser, observerLocator) {
...
const ListenerBindingRenderer = /*@__PURE__*/ renderer(class ListenerBindingRenderer {
    constructor() {
        this.target = templateCompiler.itListenerBinding;
        /** @internal */
...
const SetAttributeRenderer = /*@__PURE__*/ renderer(class SetAttributeRenderer {
    constructor() {
        this.target = templateCompiler.itSetAttribute;
    }
...
const SetClassAttributeRenderer = /*@__PURE__*/ renderer(class SetClassAttributeRenderer {
    constructor() {
        this.target = templateCompiler.itSetClassAttribute;
    }
    render(_, target, instruction) {
...
const SetStyleAttributeRenderer = /*@__PURE__*/ renderer(class SetStyleAttributeRenderer {
    constructor() {
        this.target = templateCompiler.itSetStyleAttribute;
    }
    render(_, target, instruction) {
...
const StylePropertyBindingRenderer = /*@__PURE__*/ renderer(class StylePropertyBindingRenderer {
    constructor() {
        this.target = templateCompiler.itStylePropertyBinding;
        PropertyBinding.mix();
    }
    render(renderingCtrl, target, instruction, platform, exprParser, observerLocator) {
        {
...
const AttributeBindingRenderer = /*@__PURE__*/ renderer(class AttributeBindingRenderer {
    constructor() {
        this.target = templateCompiler.itAttributeBinding;
...
const SpreadRenderer = /*@__PURE__*/ renderer(class SpreadRenderer {
    constructor() {
        ...
        /** @internal */ this._rendering = kernel.resolve(IRendering);
        this.target = templateCompiler.itSpreadTransferedBinding;
    }
    render(renderingCtrl, target, instruction, platform, exprParser, observerLocator) {
...
const SpreadValueRenderer = /*@__PURE__*/ renderer(class SpreadValueRenderer {
    constructor() {
        this.target = templateCompiler.itSpreadValueBinding;
...
class Rendering {
    ...
    render(controller, targets, definition, host) {
        ...
        if (!isHydrating && targetCount !== rowCount) {
            throw createMappedError(757 /* ErrorNames.rendering_mismatch_length */, targetCount, rowCount);
        }
        ...
        if (rowCount > 0) {
            while (rowCount > i) {
                ...
                while (jj > j) {
                    ...
                    const createsChild = instructionType === templateCompiler.itHydrateTemplateController || instructionType === templateCompiler.itHydrateElement;
...
class ComputedWatcher {
    ...
    _run() {
        ...
        if (++this._computeDepth > 100) {
            throw createMappedError(655 /* ErrorNames.watcher_infinite_loop */);
        }
...
class Controller {
    ...
    deactivate(initiator, _parent) {
        ...
        const asyncPrevActivation = kernel.isPromise(prevActivation) && initiator !== this;
...
const getAccessScopeAst = (key) => {
    ...
    if (ast == null) {
        ast = expressionParser.createAccessScopeExpression(key, 0);
...
class If {
    ...
    _swap(value) {
        ...
        let view;
        return kernel.onResolve(this.pending, () => this.pending = kernel.onResolve(currView?.isActive ? currView.deactivate(currView, ctrl) : void 0, () => {
            if (!isCurrent()) {
                return;
            }
            // falsy -> truthy
...
class Repeat {
            ...
            if (to === 'key') {
                if (command === null) {
                    ...
                    this.key = typeof value === 'string'
                        ? kernel.resolve(expressionParser.IExpressionParser).parse(value, etIsProperty)
                        : value;
                }
                else {
                ...
                if (command === null) {
                    ...
                    throw createMappedError(821 /* ErrorNames.repeat_invalid_contextual_binding_command */, command);
                }
            }
            else {
    ...
    binding(_initiator, _parent) {
        ...
        if (this._contextualExpr !== void 0) {
            const result = runtime.astEvaluate(this._contextualExpr, this.$controller.scope, binding, null);
...
class AuCompose {
    ...
    propertiesChanged(changes) {
        if (this.flushMode === 'async') {
            ...
            this._handleChangeInfo(new ChangeInfo(this.template, this.component, this.model, 'model' in changes ? 'model' : undefined));
        }
    }
    /** @internal */
    _handleChangeInfo(info) {
        this._composing = kernel.onResolve(this._composing, () => kernel.onResolve(this.queue(info, void 0), (context) => {
            if (this._contextFactory._isCurrent(context)) {
                this._composing = void 0;
            }
        }));
...
AuCompose.$au = {
    ...
    bindables: [
        ...
        { name: 'flushMode', set: v => {
                ...
                throw createMappedError(809 /* ErrorNames.au_compose_invalid_flush_mode */, v);
            } },
    ]
};
...
exports.ISSRContext = ISSRContext;
...
exports.Watch = Watch;
exports.With = With;
exports.adoptSSRView = adoptSSRView;
exports.adoptSSRViews = adoptSSRViews;
exports.alias = alias;
exports.bindable = bindable;
exports.bindingBehavior = bindingBehavior;
...
exports.customAttribute = customAttribute;
exports.customElement = customElement;
exports.findMatchingEndMarker = findMatchingEndMarker;
exports.getEffectiveParentNode = getEffectiveParentNode;
exports.hydrateSSRDefinition = hydrateSSRDefinition;
...
exports.isSSRScope = isSSRScope;
exports.isSSRTemplateController = isSSRTemplateController;
exports.lifecycleHooks = lifecycleHooks;
...
exports.partitionSiblingNodes = partitionSiblingNodes;
